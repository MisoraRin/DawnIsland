...
class SeriesContentModel(private val id: String) {
    ...
    suspend fun getSeriesContent(page: Int, next: Boolean): Any {
        var mpage = page
        //val localList=MyApplication.getDaoSession().replysBeanDao.queryBuilder().where(ReplysBeanDao.Properties.ParentId.eq(seriesId)).list()
        if (next && wholePage) {
            Log.d(TAG, "页数+1")
            mpage++
        }
    ...
    private fun preFormatJson(page: Int, s: String): List<ReplysBean> {
        ...
        if (page == 1) {
            ...
            seriesContentJson.po=po
            //Log.d(TAG, "存储前回复条数"+seriesContentJson.replys.size.toString())
            //MyApplication.getDaoSession().seriesContentJsonDao.insert(seriesContentJson)
            //Log.d(TAG,"存储后回复条数"+seriesContentJson.replys.size.toString())
